  ...
  switch (ty2.tag) {
    ...
    case "Func": {
      ...
      for (let i = 0; i < ty1.params.length; i++) {
        if (!subtype(ty2.params[i].type, ty1.params[i].type)) return false; // contravariant
      }
    ...
    case "Object": {
      ...
      for (const { name: name2, type: propTy2 } of ty2.props) {
        ...
        if (!subtype(propTy1, propTy2)) return false;
      }
      return true;
    }
  }
...
export function typecheck(t: Term, tyEnv: TypeEnv): Type {
  switch (t.tag) {
    ...
    //  if (!subtype(thnTy, elsTy)) error("then and else have different types", t);
    //  return thnTy;
    //}
    case "number":
      return { tag: "Number" };
